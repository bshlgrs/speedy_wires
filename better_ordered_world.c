...
void evolve_world(struct world * world) {
  ...
  for (i = 0; i < world->size; i++) {
    ...
    if (cell.cell_type == HEAD) {
      int x, y;
      for (x = -1; x <= 1; x += 2) {
        int pos = binary_search(cell.x - x, cell.y - 1, world);
        if (pos == -1)
          pos = binary_search(cell.x - x, cell.y, world);
        if (pos == -1)
          pos = binary_search(cell.x - x, cell.y + 1, world);

        world->cell_array[pos].neighboring_heads++;
        int i;
        for (i = 0; i < 2; i++) {
          struct cell neighbor_candidate = world->cell_array[pos+i];
          if (abs(neighbor_candidate.y - cell.y) <= 1 && neighbor_candidate.x == cell.x + x) {
            world->cell_array[pos+i].neighboring_heads++;
          }
        }
      }
    }
